export class ClassesName{
                    static CONTINER_MAIN_FOR_HOME = "continer_main_for_home";
                    static CONTINER_MAIN_FOR_ALL = 'continer_main_for_all'
                    static CONTINER_MAIN_FOR_CART = "continer_main_for_cart";
                    static DIV_CARD = "div_card";
                    static HOVER_CARD = "hover_card";
                    static DIV_PRESENTATION = "div_presentation";
                    static DIV_TRAILER = "div_trailer";
                    static DIV_GALLERY = "gallery-grid";
                    static P_PRESENTATION = "p_presentation";
                    static IFRAME_VIDUE = "vidue_iframe";
                    static SHOW_DETAILS_CONTAINER = "show-details-container";
                    static HOME_PAGE = "home_page";
                    static ABOUT = "about";
                    static CONTACT_US = "contact_us";
                    static IMG_FOR_CARD = "img_for_card";
                    static CTA_BUTTON = "cta-button";
                    static CTA_BUTTON_LARGE = 'cta-button-large';
                    static INVITATION_BUTTON = 'invitation-button';
                    static TESTIMONIAL_MINI_CARD = 'testimonial-mini-card';
}

export class Route{
    static home = "/home";
    static onouther = "/anouther";
}

export class State{
    static VIEW = {
                    ALL_PRESENTATIONS: "allpresentations",
                    CARD_PRESENTATION: "cardPresentation",
    };
}